...
type AIComment = { severity: 'HIGH' | 'MED' | 'LOW'; text: string }
...
const AI_COMMENTS: AIComment[] = [
  {
    severity: 'HIGH',
    text: 'currency param flows unsanitized into charge() — potential injection via string interpolation in charge.ts:88',
  },
  {
    severity: 'MED',
    text: 'auditLog.record() is not awaited — failures will be silently swallowed on the hot path',
  },
]

const severityColors: Record<AIComment['severity'], string> = {
...
export default function PRMockup() {
  return (
    <div className="relative border border-dashed border-accent/20 bg-surface overflow-hidden shadow-glow">
      {/* Window chrome */}
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-dashed border-border bg-surface-2">
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono text-[11px] text-fog-muted ml-1">
        ...
        <div className="space-y-2">
          {AI_COMMENTS.map((c, i) => (
            <div key={i} className="flex items-start gap-2 text-[11px]">
              <span
                className={`font-mono uppercase text-[8px] tracking-widest px-1.5 py-0.5 shrink-0 mt-0.5 ${severityColors[c.severity]}`}
              >
                {c.severity}
              </span>
